 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "    \\n",
    "    file = Path(ips_api_v1.generate_tar_from_runids(runids))\\n",
...
def initialize_jupyter_python_api(jupyterhub_dir: Path) -> None:
    ...
    python_fname = f'ips_api_{CURRENT_API_VERSION}.py'
    shutil.copyfile(
        source_dir / python_fname,
        dest_dir / python_fname,
    )

    with open(dest_dir / f'ips_api_{CURRENT_API_VERSION}_notebook.ipynb', 'wb') as f:
...
Basic API instructions:
  - ips_api.get_data() - this generates a generic IPS mapping - a mapping of floating-point timesteps to a list of data file paths (absolute). Note that your notebook will need to handle the actual loading of the data.
  - ips_api.get_child_data() - this generates a mapping of child runids to the "generic IPS mapping" described above.

To understand how the API works, you can inspect the 'ips_api_{CURRENT_API_VERSION}.py' file directly by going up one directory from where this notebook is located.
...
def _ips_api():
    ...
    _ips_spec = importlib.util.spec_from_file_location(
        '', str(_IPS_THIS_DIR.parent / 'ips_api_{CURRENT_API_VERSION}.py')
    )
    ...
    return _ips_module.IpsRunApi(_IPS_THIS_DIR)
