...
Basic API instructions:
  - `ips_analysis_api.get_data()` (returns: `dict[float, list[str]]`) - this generates a generic IPS mapping - a mapping of floating-point timesteps to a list of data file paths (absolute). Note that your notebook will need to handle the actual loading of the data.
  - `ips_analysis_api.get_child_data()` (returns: `dict[int, dict[float, list[str]]]`) - this generates a mapping of child runids to the "generic IPS mapping" described above.
  - `ips_analysis_api.get_child_data_not_ensembles()` (returns: `dict[int, dict[float, list[str]]]`)- get the child runid mapping as described above, but only use child runids NOT associated with ensembles.
  - `ips_analysis_api.get_child_data_by_ensemble_names()` (params: 1) `component_names: list[str] | None = None`, 2) `ensemble_names: list[str] | None = None`) (returns: `dict[int, dict[float, list[str]]]`) - gets the child runid mapping as described above, but will only retrieve child runids associated with ensembles. You can further filter this by ensemble name by providing an optional list of component names and an optional list of ensemble names; for example, ``ips_analysis_api.get_child_data_by_ensemble_names(ensemble_names=['ensemble_name_1', 'ensemble_name_2'])`` will ONLY fetch the child runids associated with 'ensemble_name_1' and 'ensemble_name_2', but will search all components for this.
